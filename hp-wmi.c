...
enum hp_wmi_commandtype {
	...
	HPWMI_KBRGB_GET_BRIGHTNESS	= 0x04,
	HPWMI_KBRGB_SET_BRIGHTNESS	= 0x05,
};
...
static ssize_t fourzone_onoff_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret, on, off;
	unsigned char buffer[128] = {0};
...
static ssize_t fourzone_color_show(struct device *dev, struct device_attribute *attr,
				 char *buf)
{
...
static ssize_t fourzone_color_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	int ret, zoneIdx, bufferIdx;
	...
	parser = input;
	for (parsed = strsep(&parser, " \t\n"); parsed; parsed = strsep(&parser, " "))
	{
