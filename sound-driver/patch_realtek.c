...
static void alc285_fixup_omen_dualmic(struct hda_codec *codec,
				    const struct hda_fixup *fix, int action)
{
	struct alc_spec *spec = codec->spec;
	struct auto_pin_cfg *cfg = &spec->gen.autocfg;
	...
	switch (action) {
	case HDA_FIXUP_ACT_PROBE: {
			snd_hda_jack_add_kctl(codec, 0x19, "Combo Mic Jack", false, SND_JACK_MICROPHONE, NULL);
			snd_hda_jack_add_kctl(codec, 0x1b, "Separate Mic Jack", false, SND_JACK_MICROPHONE, NULL);
			break;
		}
	}
}
...
static const struct hda_fixup alc269_fixups[] = {
	...
	[ALC285_FIXUP_OMEN_DUALMIC] = {
		.type = HDA_FIXUP_FUNC,
		.v.func = alc285_fixup_omen_dualmic,
		.chained = false
	},
};
...
static const struct snd_pci_quirk alc269_fixup_tbl[] = {
	...
	SND_PCI_QUIRK(0x103c, 0x8748, "Omen", ALC285_FIXUP_OMEN_DUALMIC),
